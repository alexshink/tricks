...
Плагин для избавления от ручного импорта не базовых компонентов:<br>
[vue-cli-plugin-import-components](https://github.com/loonpwn/vue-cli-plugin-import-components)
...
> Важно: если у изображения, которое является ребёнком flexbox-блока, есть свойство `max-width: 100%`, то необходимо задать ему `max-width: none`, иначе в некоторых случаях это может привести к тому, что изображение вообще не будет иметь ширины, то есть не отобразится.
...
Многолетний баг в Chromium, который предотвращает работу `position: fixed;` у ребёнка, если у его родителя есть свойство `backdrop-filter`. В качестве обходного решения можно делать псевдоэлемент и накидывать фильтр на него.
https://issues.chromium.org/issues/40835530
https://issues.chromium.org/issues/40942655

# Sublime Text
## экспорт/импорт настроек
Для экспорта настроек на другой компьютер можно юзать пакет [PackageSync](https://packagecontrol.io/packages/PackageSync). После установки пакета:
1. Вкладка `Tools`
2. Экспорт: `PackageSync` -> `Backup/Restore` -> `Backup packages to zip`
3. Несём zip на комп для импорта
3. Импорт: `PackageSync` -> `Backup/Restore` -> `Backup packages from zip`
